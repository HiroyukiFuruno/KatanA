...
.PHONY: check-light
check-light: fmt-check lint-impacted ## Quick verification (skip slow fixture tests; ast-lint runs inside cargo test)
	$(RTK) scripts/runner/impacted.py test
	@echo "✅ Light checks passed"


.PHONY: check
check: fmt-check lint-impacted test check-platforms ## Fast impacted verification (local default)
	@echo "✅ All checks passed"

.PHONY: check-local
check-local: fmt lint-impacted test check-platforms ## Full local verification incl. cross-platform checks
	@echo "✅ All checks passed"
