pub mod centering {
    use egui::{Rect, Response, Shape, Stroke, Ui, remap, vec2};
    ...
    const OPTICAL_Y_OFFSET: f32 = 0.1;
    ...
    impl AccordionIcon {
        ...
        pub fn paint_optically_centered(ui: &mut Ui, openness: f32, response: &Response) {
            ...
            rect.set_center(egui::pos2(
                rect.center().x,
                rect.center().y + OPTICAL_Y_OFFSET,
            ));
            ...
            let rotation =
                egui::emath::Rot2::from_angle(remap(openness, 0.0..=1.0, -TAU / 4.0..=0.0));
            for p in &mut points {
