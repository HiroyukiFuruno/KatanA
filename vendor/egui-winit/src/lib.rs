...
impl State {
    ...
    fn on_keyboard_input(&mut self, event: &winit::event::KeyEvent) {
        ...
        /* WHY: On JIS keyboards, the ¥ key (IntlYen) with Shift produces logical_key = Pipe.
        But IntlYen is physically the Backslash key. We force the physical mapping to take
        precedence so that Cmd+Shift+¥ correctly matches shortcuts expecting Backslash,
        not Pipe. Without this, Cmd+¥ and Cmd+Shift+¥ would use different egui::Key values
        and the Shift variant would never match its registered shortcut. */
        let logical_key = if matches!(
            event.physical_key,
            winit::keyboard::PhysicalKey::Code(winit::keyboard::KeyCode::IntlYen)
        ) {
...
fn key_from_key_code(key: winit::keyboard::KeyCode) -> Option<egui::Key> {
    ...
    Some(match key {
        ...
        /* WHY: On JIS keyboards, the ¥ key reports KeyCode::IntlYen (scancode 0x5d)
        instead of KeyCode::Backslash. Map it to Backslash so that Cmd+¥ shortcuts
        work identically to Cmd+\ on US/EU layouts. */
        KeyCode::IntlYen => Key::Backslash,
