...
## [0.22.25] - 2026-05-20 01:09:41 (JST)

### 🐛 不具合修正

- **版上げ（version bump）時も品質チェックが必ず走るように修正**: バージョン更新のリリース作業で、継続的インテグレーション（CI）を回避できてしまう問題を解消しました。アプリのバージョンに触れるリリース PR は、マージ前に通常の検証経路を通ります。
- **言語設定が OS の言語に追従できるように修正**: 言語設定に「自動（Auto / Follow OS）」を追加し、新規環境ではこの設定を既定にしました。既に日本語や英語などを明示選択している場合は、その保存値を勝手に変更しません。
- **更新確認エラーの詳細を各言語で表示**: 更新確認ダイアログ（dialog）に、通信ライブラリの英語エラーがそのまま出る問題を修正しました。接続失敗、タイムアウト（timeout）、サーバー応答、プロキシ（proxy）、応答形式、原因不明の各エラーを、対応済みのすべてのアプリ言語で表示します。

## [0.22.24] - 2026-05-19 (JST)

### 🐛 不具合修正

- **起動時の「更新の確認に失敗しました」エラー表示を解消**: 自動の更新確認ダイアログが、起動毎に「更新の確認に失敗しました — io: Connection refused」を赤字で表示する不具合を修正しました（特に Windows で、ureq が環境変数 / システムから自動採用したプロキシ（proxy）の接続先が到達不能だった場合に発生）。更新確認は GitHub Releases API へ直接接続を先に試み、それが失敗した場合のみ明示的に設定されたプロキシへフォールバックします。最終的に取得できなかった場合（接続拒否・DNS 失敗・レート制限・不正なレスポンス等）は、「現時点で更新情報を取得できなかった」として静かに扱い、起動時 UI に赤いエラーダイアログを出さなくなります。

## [0.22.23] - 2026-05-19 (JST)

### 🐛 不具合修正

- **Windows のコンソールウィンドウ表示抑止**: KatanA が PlantUML 描画用に Java を起動した際、Windows 環境で一瞬コンソールウィンドウ（console window）が表示される問題を修正しました（特に起動時に開いている文書に PlantUML ブロックが含まれていると顕著でした）。`java.exe` を `CREATE_NO_WINDOW` フラグ強制パスのファサード経由で起動するようにし、Java の子プロセスにコンソールが割り当てられないようにしています。

### 🔧 その他

- **ヘッドレス（headless）ファサードの強化**: プロセス起動ファサードから抜け穴になっていた `create_command_visible` API を撤去し、Windows 上のすべての外部プロセス起動が `CREATE_NO_WINDOW` 強制パスを通るようにしました。AST Linter のメッセージも、外部プロセス起動の正規ルート（`ProcessService::create_command`）を案内する内容に更新しています。
...
- `ExplorerPanel` および `ExplorerContent` を builder パターン（`with_referenced_images()`）にリファクタリングし、引数数の linter 上限に対応しました。
...
- **Markdown LinterのUI統合と自動修正 (Quick Fix) 機能**:
  ...
  - AST Linterの制約に準拠するため、markdownlintルールのファイル分割・リファクタリングを実施しました。
...
- ショートカット設定の重複を開発時に静的検知するASTリンターを新設し、設定ファイルの堅牢性を向上させました。
