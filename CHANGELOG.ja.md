...
## [0.22.41] - 2026-08-26 15:32:56 (JST)

### 🚀 新機能

- **Excelのシートタブ**: プレビュー下部に表示されるシート名タブから、Excelのワークシートを切り替えられるようになりました。
- **PDFの目次**: PDFに埋め込まれた階層目次を表示し、選択したページへ直接移動できるようになりました。

### ✨ 改善

- **開いているファイルの表示**: PDFまたはOffice文書を開くと、ワークスペースのエクスプローラーで対応するファイルを選択して表示するようになりました。
- **文書に応じた操作**: 意味のある目次を持たないWord、Excel、PowerPoint文書では、目次ボタンを非活性にしました。

### 🐛 不具合修正

- **HTML文書の操作**: HTMLの描画後にJavaScript操作、ポインター入力、スクロールが効かなくなる不具合を修正しました。
- **Office文書の安定性**: 固定境界をまたぐ結合セル、無効化されたアクティブコンテンツ、ZIPデータ記述子、大容量ワークシート、PowerPoint表のセル結合、壊れた埋め込み画像による表示失敗を修正しました。

## [0.22.40] - 2026-08-21 18:45:51 (JST)

### 🐛 不具合修正

- **PowerPoint の外部リンク**: 標準の外部ハイパーリンクを含む PowerPoint ファイルを正常に開けるようにし、プレビュー中はリンク先へ接続しない安全性を維持しました。
...
- **PDF / Office文書の表示**: ローカルファイルとHTTP/HTTPSの直接URLから、PDF、Word、Excel、PowerPoint文書をKatanAの文書タブで開けるようにしました。
- **ページと表計算の操作**: PDFとWordのページ、PowerPointのスライド、Excelのシートをプレビューから切り替え、表計算のセルをスクロール可能な表で表示できます。

### ✨ 改善

- **文書の操作部品**: 形式ごとの対応機能に合わせて、前後移動、拡大縮小、ページ全体への調整、幅への調整、選択セルのコピーを共通の操作で利用できます。
- **原因を追える文書エラー**: 読み込み元、ネットワーク、形式、表示処理の失敗について、操作、文書形式、場所、元の原因を表示し、作業中のワークスペースを空白の画面で置き換えないようにしました。
- **Office文書の安全なプレビュー**: Officeマクロや埋め込みスクリプトを実行せず、外部コンテンツが拒否された場合は無断で取得せず診断として示します。

### 🔧 その他

- **バックグラウンド表示の安定性**: 大きな文書の読み込み、描画、ページ移動、終了と連続操作がアプリ画面を停止させないようにしました。

## [0.22.37] - 2026-07-29 05:30:18 (JST)

### 🚀 新機能

- **インタラクティブHTMLビューアー**: ローカルファイルとHTTP/HTTPS文書をKatanA内で開き、CSS、JavaScript、同一オリジンのリソースとiframe、リンク、文字入力、ポインターまたはキーボード操作を利用できるようにしました。

### ✨ 改善

- **ブラウザ互換レイアウト**: HTMLスライドで、表示領域基準のサイズ、複数列とflexレイアウト、グラデーション、固定操作部品、明示的な改行、多言語文字、円内の数字、ページ送り操作部品を適切に配置します。
- **応答性の高いHTML操作**: 高頻度のポインター移動とスクロール中も、クリック、キーボード入力、ナビゲーション、後続フレーム更新を妨げず、HTMLの初回表示までに生じていた不要な遅延を解消しました。
- **原因を追えるHTMLエラー**: 画面上の診断に失敗した操作、文書、根本原因を表示し、HTTPステータス、サーバーのブラウザ検証、JavaScript実行失敗を区別できるようにしました。

### 🐛 不具合修正

- **文書切り替え**: 古いリモートURLの応答が後から完了しても、その後に開いたローカルHTMLを置き換えないようにし、操作時に前の文書の古いフレームを再利用する問題も修正しました。
- **ローカルHTML URLとスライドショー**: `file://` URLのクエリとフラグメントを維持し、同じフォルダーのiframeを読み込み、インタラクティブなスライドを進められるようにしました。iframeファイルがない場合も、空白の代わりに具体的なエラーを画面内へ表示します。
- **画像表示操作部品**: 最大化中の拡大状態を上下左右の移動後も維持し、ライトテーマや白い画像でも操作部品を識別できる固定の高コントラスト背景と枠線を適用しました。

## [0.22.36] - 2026-07-23 00:29:23 (JST)

### ✨ 改善

- **ブラウザー互換のHTMLスタイル**: HTMLページで、構造化されたスタイルの優先順位、カスタムプロパティ、レスポンシブ規則、flex/gridレイアウト、box sizing、overflow、表、文字組み、外部画像、埋め込みSVGをより正確に適用するようにしました。
- **Web URL文書**: 入力したWebページのリダイレクト後URLを維持し、そのURLを基準に相対スタイルシート、スクリプト、画像を読み込み、ページ内リンクをアクティブタブの履歴へ反映するようにしました。

### 🐛 不具合修正

- **JavaScriptイベント伝播**: click handlerで `stopPropagation()` や `preventDefault()` を使用し、capture、target、bubbleの順序で処理できるようにしました。不正なevent objectによる停止も解消しました。
- **HTML表示領域の全面描画**: HTML文書の外側にアプリ固有の枠、境界線、余白、背景を追加せず、ページのpixelがpreview領域全体を埋めるようにしました。

## [0.22.35] - 2026-07-22 11:09:02 (JST)

### 🐛 不具合修正

- **HTML文書のライフサイクルイベント**: `DOMContentLoaded`、`readystatechange`、`load` を登録するスクリプトが、文書イベントAPI不足で停止せず正しく初期化されるようにしました。
- **追跡可能なライフサイクルエラー**: 文書ライフサイクル中のスクリプト例外が、汎用的なブラウザーワーカー停止に置き換わらず、文書URL、操作、スクリプトの行・列、JavaScript stackまで表示されるようにしました。
- **GitHub API上限到達時の更新確認**: 未認証GitHub APIがステータス403または429を返した場合、公開リリースURLから最新版を確認するようにし、共有IPの利用上限で更新確認が停止しないようにしました。

### 🔧 その他

- **ネイティブHTML描画の安定性向上**: 外部ブラウザーに依存しないアプリ構成を維持したまま、共通のRust / V8描画経路を更新しました。
- **リリースアセット検査**: ZIP内の空白を含むパスを保持して検査し、正常なmacOS app bundleを不正と誤判定しないようにしました。

## [0.22.34] - 2026-07-22 03:23:10 (JST)

### ✨ 改善

- **HTMLリソースの完全な描画**: ローカルHTMLファイルとWebページで、許可された外部スタイルシート、スクリプト、画像、埋め込みSVGを読み込めるようになりました。個別のリソースが拒否または失敗しても、主文書の表示は維持されます。
- **追跡可能なHTMLエラーと回復**: HTMLエラーに失敗した層、操作、文書URL、一次原因を表示し、ブラウザーセッションの起動失敗後も同じプロセス内でビューアーを回復できるようになりました。

### 🐛 不具合修正

- **画像の全画面移動**: 拡大した画像を上下左右へ移動またはスクロールしても、全画面表示と選択した寸法を維持するようにしました。文書の自動更新が発生した場合も維持されます。
- **明るい画像上の操作アイコン**: 全画面、移動、拡大縮小、閉じる操作に固定の暗い背景と明瞭な境界線を付け、ライトテーマや白背景画像でも見えるようにしました。

### 🔧 その他

- **HTML描画の安定性向上**: 共通のRust / V8描画経路とクロスプラットフォームのリリース検証を更新し、外部ブラウザーへ依存しない構成を維持しました。

## [0.22.33] - 2026-07-04 14:26:11 (JST)

### 🚀 新機能

- **操作可能なHTMLブラウザー表示**: ローカルの `.html` / `.htm` ファイルとWeb URLをネイティブの文書ビューで開き、CSSレイアウト、JavaScriptの操作、フォーム入力、リンク、ページ遷移、再読み込みをブラウザー相当で確認できるようになりました。

### ✨ 改善

- **ワークスペースでのファイル操作**: HTMLファイルがワークスペースに表示され、ファイル選択やドラッグ＆ドロップから開けるようになりました。通常のワークスペースフィルターもそのまま適用されます。

### 🐛 不具合修正

- **Markdown専用機能の対象範囲**: HTMLファイルがMarkdown診断、整形、文書出力の対象として扱われないようにし、プレビュー専用文書で誤解を招くメッセージが出にくくなりました。

### 🔧 その他

- **プレビューの安定性向上**: HTML操作後の更新内容と変化していないページ内容が同時に残り、文字の重なりや欠けがないことを自動検証するようにしました。

## [0.22.32] - 2026-07-03 21:49:42 (JST)

### 🐛 不具合修正

- **変更後のエディター応答性**: 文書の編集、保存、Markdown lint の修正適用後に、文書更新のタイミングでエディターがフリーズしたように見える回帰を修正しました。
...
- **PDF出力のレイアウト、リンク、テーブル、コードブロック、テーマ**: PDF出力で日本語の長い段落が重なる問題を修正し、KDV出力経路でMarkdownのリンクとテーブル表現を保持したうえで、ダークテーマのコードブロックのコントラストと長いリンクの注釈範囲を修正し、PDFのテーマ処理を現在の文書テーマに合わせ、PDFの脚注ジャンプ、OS依存の絵文字、テーブル内inline codeの表示もHTML / viewer出力と揃うようにしました。

### 🔧 その他

- **依存関係の更新**: このリリースに、katana-document-viewer 0.2.5、egui / eframe 0.35、egui_commonmark 0.24、プレビューとテスト周辺の補助依存関係更新を含めました。

## [0.22.29] - 2026-05-28 10:31:22 (JST)

### 🐛 不具合修正

- **Draw.io 図形内テキストの配置**: プレビュー内の複数行 Draw.io ラベルで、枠内テキストの先頭に不要な空白行が入ったように見える問題を修正しました。

## [0.22.28] - 2026-05-27 16:02:47 (JST)
...
## [0.22.27] - 2026-05-27 08:48:35 (JST)
...
## [0.22.26] - 2026-05-26 04:14:28 (JST)
...
## [0.22.25] - 2026-05-20 02:13:21 (JST)
...
## [0.22.24] - 2026-05-19 18:15:55 (JST)
...
## [0.22.23] - 2026-05-19 16:44:56 (JST)
...
## [0.22.22] - 2026-05-18 17:22:37 (JST)
...
## [0.22.16] - 2026-05-13 00:20:29 (JST)
...
## [0.22.13] - 2026-05-10 18:30:00 (JST)
...
## [0.22.12] - 2026-05-09 00:04:10 (JST)
...
## [0.22.11] - 2026-05-04 22:29:09 (JST)
...
- **更新ログ出力**: `%LOCALAPPDATA%\KatanA\update.log` に `evacuate` / `replace` / `launch` / `rollback` の各フェーズを記録する処理を追加しました。
- **旧版ユーザー向け案内**: `v0.22.10` の Linux / Windows ユーザーが新規更新で詰まる場合向けの手動更新手順を追記しました。

## [0.22.10] - 2026-05-03 09:27:30 (JST)
...
## [0.22.9] - 2026-04-29 09:20:53 (JST)
...
## [0.22.8] - 2026-04-28 14:55:06 (JST)
...
## [0.22.7] - 2026-04-27 14:24:43 (JST)
...
## [0.22.6] - 2026-04-25 23:03:32 (JST)
...
## [0.22.5] - 2026-04-25 16:59:44 (JST)
...
## [0.22.4] - 2026-04-24 06:40:00 (JST)
...
## [0.22.3] - 2026-04-22 10:49:58 (JST)
...
- `file.close_workspace` や `view.refresh_explorer` 等の複数アクションに新しいデフォルトショートカットを割り当てました。
...
## [0.22.2] - 2026-04-21 01:38:00 (JST)
...
## [0.18.1] - 2026-04-10 00:40:06 (JST)
...
## [0.16.7] - 2026-04-07 10:45:00 (JST)
...
## [0.16.3] - 2026-04-03 18:50:00 (JST)
...
## [0.13.0] - 2026-04-02 19:15:00 (JST)
