...
- 【[OpenSpec - Tab Groups & Editor Features](./openspec/changes/v0-11-0-tab-groups-and-editor-features/proposal.md)】アプリケーション再起動時のタブ復元（セッション永続化）ロジックを刷新し、タブグループの所属状況やピン留め状態といった高度な情報も完全な状態で引き継がれるようになりました。

## [0.10.1] - 2026-04-01 18:35:00 (JST)

### ✨ 改善

- 【[OpenSpec - Diagram Color Sync](./openspec/changes/archive/2026-04-01-v0-10-1-diagram-color-sync/proposal.md)】テーマやプレビュー文字色を変更した際、アプリを再起動することなく即座にダイアグラムのプレビューへ色が反映されるよう改善しました。
...
- 【[OpenSpec - Cache Optimization](./openspec/changes/v0-10-0-cache-io-optimization/README.md)】文字入力やタブ切り替え時のキャッシュ保存・ディスク書き込み処理を最適化し、長時間の連続使用時等に発生しやすかったUIのもたつきやフリーズを解消しました。
...
- CI/CDパイプラインを安定させるため、Clippyが検知した複数の静的解析エラーを修正しました。

## [0.8.8] - 2026-03-30 23:30:00 (JST)

### 🔧 システム / 内部改善

- マークダウンプレビューエンジンの依存先（egui_commonmarkおよび関連ライブラリ）の管理方式を Git の `subtree` 構造へ移行し、将来の保守性とパッチ分離性を向上しました。
...
- モノリシックなUIモジュール（`workspace`、`editor`、`top_bar`）を `ui.rs` と `logic.rs` に分離し、テスト容易性と保守性を向上。
- 純粋なロジック関数（タブのドラッグ＆ドロップ計算、スクロール同期、行番号-文字インデックス変換、カラー解決）を専用のテスト可能なモジュールに抽出し、包括的なユニットテストを追加。
- ワークスペースパネルの7ファイルを2つの凝集度の高いモジュール（ui + logic）に統合し、コードの断片化を解消。

### 🧪 テスト

- エディタロジック（スクロール同期、行マッピング、ハイライトカラー）およびタブバーロジック（ドロップポイント計算、ドラッグ＆ドロップ解決、タイトルフォーマット）のユニットテストを追加。
