...
The resolved version of KDV `0.5.2` and minimum KRR `0.4.15` is required.
...
write_fixture "$pass_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
expect_accept "published browser dependency lines and patch floors" "$pass_dir"

future_kdv_patch_dir="$TMP_ROOT/future-kdv-patch"
write_fixture "$future_kdv_patch_dir" '"0.5.3"' '"0.4.15"' 0.5.3 0.4.15 true 0
expect_reject "KDV patch beyond the v0.22.38 release contract" "$future_kdv_patch_dir"

future_kdv_line_dir="$TMP_ROOT/future-kdv-line"
write_fixture "$future_kdv_line_dir" '"0.6.0"' '"0.4.15"' 0.6.0 0.4.15 true 0
expect_reject "incompatible KDV release line" "$future_kdv_line_dir"

stale_kdv_lock_dir="$TMP_ROOT/stale-kdv-lock"
write_fixture "$stale_kdv_lock_dir" '"0.5.2"' '"0.4.15"' 0.4.2 0.4.15 true 0
expect_reject "stale KDV dependency patch" "$stale_kdv_lock_dir"

stale_krr_lock_dir="$TMP_ROOT/stale-krr-lock"
write_fixture "$stale_krr_lock_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.14 true 0
expect_reject "stale KRR dependency patch" "$stale_krr_lock_dir"

stale_acceptance_lock_dir="$TMP_ROOT/stale-acceptance-lock"
write_fixture "$stale_acceptance_lock_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
python3 - "$stale_acceptance_lock_dir/scripts/screenshot/Cargo.lock" <<'PY'
import sys
from pathlib import Path

path = Path(sys.argv[1])
path.write_text(path.read_text().replace('version = "0.5.2"', 'version = "0.4.2"', 1))
PY
...
write_fixture "$missing_checksum_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$stale_manifest_dir" '"0.4.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
expect_reject "manifest permits stale dependency patches" "$stale_manifest_dir"

path_dependency_dir="$TMP_ROOT/path-dependency"
write_fixture "$path_dependency_dir" '{ version = "0.5.2", path = "../kdv" }' '"0.4.15"' 0.5.2 0.4.15 true 0
expect_reject "local path dependency" "$path_dependency_dir"

git_dependency_dir="$TMP_ROOT/git-dependency"
write_fixture "$git_dependency_dir" '"0.5.2"' '{ version = "0.4.15", git = "https://example.invalid/krr" }' 0.5.2 0.4.15 true 0
expect_reject "git dependency" "$git_dependency_dir"

patch_override_dir="$TMP_ROOT/patch-override"
write_fixture "$patch_override_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$cargo_config_override_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_resource_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
mv \
...
write_fixture "$missing_semantic_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_loopback_open_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_http_requests_assert_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_stop_propagation_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_advanced_css_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_lifecycle_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_horizontal_pan_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_fragment_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_origin_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$missing_raw_frame_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$ascii_only_input_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$fixed_click_bounds_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$native_request_name_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
...
write_fixture "$native_runner_dir" '"0.5.2"' '"0.4.15"' 0.5.2 0.4.15 true 0
