...
### Requirement: v0.22.37 release must prove the published browser chain
The minimum resolved version of KDV `0.3.5` and KRR `0.4.14` is required.
...
expect_accept() {
    local name=$1
    local directory=$2
    if ! KATANA_RELEASE_ROOT="$directory" bash "$GUARD" 0.22.37 >/dev/null; then
...
write_fixture "$missing_checksum_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$stale_manifest_dir" '"0.3.0"' '"0.4.0"' 0.3.5 0.4.14 true 0
expect_reject "manifest permits stale dependency patches" "$stale_manifest_dir"

path_dependency_dir="$TMP_ROOT/path-dependency"
write_fixture "$path_dependency_dir" '{ version = "0.3.5", path = "../kdv" }' '"0.4.14"' 0.3.5 0.4.14 true 0
expect_reject "local path dependency" "$path_dependency_dir"

git_dependency_dir="$TMP_ROOT/git-dependency"
write_fixture "$git_dependency_dir" '"0.3.5"' '{ version = "0.4.14", git = "https://example.invalid/krr" }' 0.3.5 0.4.14 true 0
expect_reject "git dependency" "$git_dependency_dir"

patch_override_dir="$TMP_ROOT/patch-override"
write_fixture "$patch_override_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$cargo_config_override_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_resource_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
mv \
...
write_fixture "$missing_semantic_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_loopback_open_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_http_requests_assert_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_stop_propagation_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_advanced_css_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_lifecycle_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_horizontal_pan_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_fragment_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_origin_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$missing_raw_frame_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$ascii_only_input_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$fixed_click_bounds_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$native_request_name_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
write_fixture "$native_runner_dir" '"0.3.5"' '"0.4.14"' 0.3.5 0.4.14 true 0
...
if KATANA_HTML_UI_SOURCE_ROOT="$forbidden_runtime_source_dir" \
    bash "$ROOT_DIR/scripts/release/check-html-browser-runtime-contract.sh" 0.22.37 \
    >/dev/null 2>&1; then
...
if ! bash "$ROOT_DIR/scripts/release/check-html-browser-runtime-contract.sh" 0.22.37 >/dev/null; then
