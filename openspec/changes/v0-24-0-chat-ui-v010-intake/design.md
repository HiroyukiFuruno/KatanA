...
`v0.23.0` で local LLM provider を user が設定・選択できる前提が整うと、次はその provider を文書生成へ広げる必要がある。現行 KatanA には次の土台がある。
...
一方で、generation の出力先を current document / new file / template scaffold の 3 系統で統一的に扱う job model はまだ存在しない。ユーザー要望では 3 つを同時に実装したいので、`v0.24.0` では出力 target ごとの個別実装ではなく、共通の generation contract を先に定義する。
