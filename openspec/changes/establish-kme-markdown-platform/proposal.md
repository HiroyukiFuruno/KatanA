...
- viewerはFloem前提のネイティブ表示でKMEモデルを消費する
...
- KME public DTOとmetadata APIが固定されるまで、KDV、KLE、KCF、KatanA統合、KUWは独自document modelや独自metadata schemaを作らない
...
- `split-scroll-sync`: KMEのsource rangeとstable node idをKatanAの同期材料として使う
- `markdown-export`: KDVがKMEモデルと解決済みmetadataを使うviewer/export pipelineを定義する
...
- `katana-canvas-forge`: 外部描画。既存exportはKDV移譲まで維持
