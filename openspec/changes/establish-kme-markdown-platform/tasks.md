...
- [x] P0 `katana-ast-lint`、P1 `katana-markdown-engine`、P2 `katana-ui-widget`、P3 downstream統合の順序が合意済みである
- [x] Floemをviewer / editor / widgetの前提にする方針が合意済みである
- [x] KCFは外部描画へ縮小し、既存exportはKDV移譲まで維持する方針が合意済みである

### Tasks

- [x] 0.1 KMEをMarkdown文書モデル、source mapping、metadata target解決の正本として固定する
...
- [x] 0.5 KMEのpublic DTOとmetadata APIが固定されるまで、downstreamが独自document modelやmetadata schemaを作らないことを各repo OpenSpecへ反映する

### Definition of Done

- [x] 別セッションが親OpenSpecからrepo別の開始条件、完了条件、停止条件を判断できる
- [x] downstream repoがKMEより先に仕様を確定してよい範囲と、待つべき範囲を判断できる

## 1. Platform Contract

### Definition of Ready

- [x] `assets/fixtures/sample.md` とREADME badgeをKME v0の仕様fixtureとして扱う合意がある
- [x] alert、description list、table/grid、footnote、emoji、diagram、math、HTML blockをKatanA現行仕様の棚卸し対象にする
- [x] CommonMark完全準拠をKME v0の完了条件にしない方針が合意済みである

### Tasks

- [x] 1.1 KME v0の対象を「現在KatanAで実現できているMarkdown挙動の踏襲」として固定する
- [x] 1.2 `sample.md`、README badge、alert、description listのfixture責務を明文化する
- [x] 1.3 CommonMark完全準拠をv0の完了条件にしないことを明記する
- [x] 1.4 KME、editor、viewer、export、KatanA統合、widget分離のrepository責務を固定する
...
- [x] 依存方向がKME中心の一方向として説明されている

## 2. Repository DoR / DoD

### Definition of Ready

- [x] `katana-ast-lint` が分離済みで、KMEから利用できる
- [x] `katana-markdown-engine` repositoryが作成済みである
...
- [x] 2.2 `katana-markdown-engine` のDoR/DoDを、文書モデル、metadata schema、位置解決、parser境界として定義する
- [x] 2.3 `katana-ui-widget` のDoR/DoDを、KME metadata/display DTO確定後のFloem共通UI部品として定義する
- [x] 2.4 `katana-document-viewer` のDoR/DoDを、KME public DTOとKUW境界待ちとして定義する
- [x] 2.5 `katana-language-editor` のDoR/DoDを、KME metadata schemaとtarget resolution API待ちとして定義する
- [x] 2.6 `katana-canvas-forge` のDoR/DoDを、外部描画と既存export維持、KDV移譲後のexport削除条件として定義する
...
- [x] KME未完了時に進めてよい作業が、repoごとに限定されている

## 3. Metadata and Editor Contract

### Definition of Ready

- [x] KMEがmetadata schemaと位置解決を持つ方針が合意済みである
...
- [ ] 3.6 conflicted metadata targetをKME public DTOとして扱い、editorやviewerが独自状態で代替しない

### Definition of Done

- [ ] metadata contractがKME、editor、viewer、exportの各OpenSpecへ展開できる
...
- [x] 4.3 `katana-markdown-engine` に `bootstrap-kme-document-model` を作る
...
- [x] 4.9 各OpenSpecへKME完了前のpending条件と禁止事項を再反映する

### Definition of Done

- [x] 各OpenSpecにDoR/DoDがある
- [x] KME本体実装と品質ゲート構築が混ざっていない
...
- [x] 5.3 KME repository作成とOpenSpec展開の差分を確認する
