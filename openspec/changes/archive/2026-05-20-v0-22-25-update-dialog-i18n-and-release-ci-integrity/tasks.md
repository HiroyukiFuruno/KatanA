...
- [x] E-1. `CHANGELOG.md` / `CHANGELOG.ja.md` に v0.22.25 entry を追加。Bug Fixes (B, C, D) を分けて記述。CHANGELOG.md は **英語のみ** で書き、UI 文言の引用も英語に統一する。
- [x] E-2. `scripts/release/check-pr-ready.sh 0.22.25` を通過する。
- [x] E-3. release notes の生成元と CHANGELOG.md が一致することを `scripts/release/extract-notes.sh 0.22.25` で確認する。`gh release view v0.22.25` は release workflow 実行後の配布確認で実施する。

#### E verification

- [x] `scripts/release/extract-notes.sh 0.22.25` が `CHANGELOG.md` の v0.22.25 section を抽出することを確認。
- [x] `gh release view v0.22.25 --repo HiroyukiFuruno/KatanA --json tagName,name` は release workflow 実行前のため `release not found` を返すことを確認。配布後確認では同じ tag の release body を確認する。
- [x] `scripts/release/check-pr-ready.sh 0.22.25` は版番号、CHANGELOG、Linuxbrew formula contract まで通過し、OpenSpec task completion の E-2 未完了のみで停止することを確認。
- [x] E-2 完了記録後に `scripts/release/check-pr-ready.sh 0.22.25` を再実行し、mechanical pre-PR checks が通過。

#### User review / visual verification

- [x] `just run-release` で KatanA を起動し、起動時に更新確認エラーダイアログが出ないことを確認。
- [x] 設定画面の「動作」タブで言語ドロップダウンを開き、「自動 (OS に追従)」が選択肢として表示されることを確認。
- [x] スクリーンショットを `tmp/qa/v0.22.25/01-launch.png`, `tmp/qa/v0.22.25/03-settings-behavior.png`, `tmp/qa/v0.22.25/04-settings-language-dropdown.png` に保存。
