...
- **WHEN** a developer adds `std::process::Command::new(...)` to any `crates/*/build.rs`
...
- **WHEN** `target_dirs` or `collect_build_scripts` accidentally drops `scripts/screenshot/src/` or any `crates/*/build.rs`
- **WHEN** `cargo test -p katana-linter` runs
