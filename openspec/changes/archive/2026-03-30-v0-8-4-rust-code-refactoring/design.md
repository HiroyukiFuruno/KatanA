...
Task 1-3（linter/core まで）はすでに実施済みだが、文書整合の観点で次の2点を確認した。
...
1. **Task 1 の仕様文書に stale な実装状況表が残っていた**
...
- UIの主要導線を、Reactコンポーネントのような再利用可能で再現性の高い自己完結コンポーネントへ再構成する
...
- 単に `render_*` free function を別ファイルへ移すだけでは **未完了** とする
- 分割先は、`Props` に相当する `struct`、描画の `show(...)` エントリポイント、ユーザー操作を返す typed response / action を持つ
- 親コンポーネントは子コンポーネントへ最小限の依存だけを渡し、巨大な `AppState` 全体を横流ししない
- `menu`, `workspace`, `tab_bar`, `settings`, `preview`, `modals`, `widgets` の release-critical UI 導線は、最終的にこのパターンで統一する

**適用順序（UI最終フェーズ）:**

1. `KatanaApp` / `AppState` の God Object を解体する
2. `shell_ui.rs`, `settings_window.rs`, `preview_pane_ui.rs`, `widgets.rs` を責務ごとに分割する
3. 分割後の各 UI モジュールを、React的な自己完結コンポーネントへ統一する
4. release-critical UI 導線の統合テストで component 境界が破綻していないことを最終確認する
