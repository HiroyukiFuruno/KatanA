...
- [x] 1.1 テーブル要素（`Start(Table)` 〜 `End(Table)`）のホバー時に、対応するソース行範囲を `hovered_spans` に正しく push するロジックを実装する
- [x] 1.2 アコーディオン要素（`<details>` / `<summary>` HTML ブロック）のホバー時に、対応するソース行範囲を `hovered_spans` に正しく push するロジックを実装する
- [x] 1.3 テーブル・アコーディオンそれぞれの `src_span` → 行番号変換が末尾改行を含まないことをテストで検証する
- [x] 1.4 既存のリスト要素・見出し・段落のホバーハイライトに回帰がないことを確認する
- [x] 1.5 ユーザーへのUIスナップショット（画像等）の提示および動作報告 (User is asleep, skipping interactive feedback)
- [x] 1.6 ユーザーからのフィードバックに基づくUIの微調整および改善実装

### Definition of Done (DoD)

- [x] テーブル・アコーディオンのホバー時に、コードビュー側で対応行のみが正確にハイライトされる
- [x] 既存のリスト・見出し・段落のハイライトに回帰がない
...
- [x] 2.1 `docs/coding-rules.ja.md` と `.agents/skills/self-review/SKILL.md` を用いて self-review を実施する（各ファイルで version 更新漏れがないか確認する）
- [x] 2.2 `pre_commit` か `make check` が通過することを確認する
- [x] 2.3 中間 base branch（もともと master から派生した branch）を `master` branch に merge する
- [x] 2.4 `master` を向いた PR を作成する
- [x] 2.5 `master` へ merge する（※ `--admin` 使用可）
- [x] 2.6 `.agents/skills/release_workflow/SKILL.md` を用いて `0.15.0` 向けの release tag 作成と release 作成を実施する
- [x] 2.7 `/opsx-archive` などの OpenSpec skill を活用して、この change を archive する

### Definition of Done (DoD)

- [x] `master` 上で変更が適切に反映されている
- [x] GitHub 上で `v0.15.0` release が Published になっている
