...
ユーザが任意のショートカットキーまたはUI上のボタンを介して呼び出したとき、ワークスペース全体からファイルを検索するためのモーダル（ポップアップウィンドウ）を表示しなければならない。
...
モーダル内で単語を入力すると、現在読み込まれているワークスペース内のインデックス（キャッシュ等）から相対パスの部分一致（または正規表現）検索を行い、結果リストを表示しなければならない。
