...
このまま shortcut customization へ進むと、menu、palette、shortcut editor が別々の command 名・availability・grouping を持つことになる。`v0.20.0` ではまず command inventory を正規化する。
...
### 2. `v0.20.0` は File / View / Help を primary scope とする
...
- 採用理由:
  ...
  - scope が分かりやすい
- 代替案:
  - 全 menu を同時に設計する: `v0.21.0` を押し下げるため不採用
...
- Window / Tools 相当を `v0.20.0` に含めるかは後続判断でよいか
