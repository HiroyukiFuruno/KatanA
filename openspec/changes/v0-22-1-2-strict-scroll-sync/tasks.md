...
- **標準（Base）ブランチ**: `release/v0.22.1-2`
- **作業ブランチ**: `feature/v0.22.1-2-task-x` (xはタスク番号)
...
- [ ] 1.1 `ScrollMapper` に文末（EOF）アンカーの明示的なサポートを追加
- [ ] 1.2 段落・ブロックレベルのアンカー登録インターフェースの実装
- [ ] 1.3 高密度アンカーを効率的に処理するための補完ロジックの改善

### Definition of Done (DoD)

- [ ] ユニットテストによる境界条件（EOF等）の検証
- [ ] `/openspec-delivery` ワークフローを実行して全デリバリ手順を完了する

## 2. UI Integration (Editor & Preview)

### Definition of Ready (DoR)

- [ ] タスク1のデリバリサイクルが完了し、`release/v0.22.1-2` に統合されていること。

- [ ] 2.1 Markdown レンダラから全ブロック要素の同期アンカーを送出するよう拡張
- [ ] 2.2 エディタ側に適応型下部パディング（Ghost Space）を実装
- [ ] 2.3 プレビュー側に適応型下部パディング（Ghost Space）を実装
- [ ] 2.4 ユーザーへのUIスナップショット（画像等）の提示および動作報告
- [ ] 2.5 ユーザーからのフィードバックに基づくUIの微調整および改善実装

### Definition of Done (DoD)

- [ ] 実機でのスクロール連動の滑らかさと精度の確認
- [ ] `/openspec-delivery` ワークフローを実行して全デリバリ手順を完了する
...
- [ ] 3.3 `release/v0.22.1-2` から `master` への PR を作成
