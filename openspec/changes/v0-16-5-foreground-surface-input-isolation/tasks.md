...
- [x] 2.1 `docs/coding-rules.ja.md` と `.agents/skills/self-review/SKILL.md` に基づいて自己レビューを実行する（各ファイルでバージョンの更新漏れがないかチェックする）
- [x] 2.2 `make check` コマンドがエラーゼロ（exit code 0）で通過することを確認する
- [x] 2.3 中継用の中間ベースブランチ（元はmasterから派生したもの）を `master` ブランチにマージする
- [x] 2.4 `master` へのPull Request（PR）を作成する
- [x] 2.5 master にマージする（※ `--admin` の使用を許可する）
- [x] 2.6 `.agents/skills/release_workflow/SKILL.md` に従って、`0.16.5` のリリースタグ生成とリリース作成を実行する
