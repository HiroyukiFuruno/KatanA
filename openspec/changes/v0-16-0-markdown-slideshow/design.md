...
### 制約の発見と是正 (Task 2 調査段階)

**発見事項:**
既存の印刷/PDFエクスポート(`pdf.rs`)における「自動ページ分割」は、Markdown や UI の Rust レベルで行われているのではなく、`headless_chrome` によるブラウザの印刷レイアウトエンジン（PDF変換機能）に全面的に依存していることが判明した。そのため、Rust 側の `katana-core` にそのまま再利用できる「ページ分割契約（API）」は存在しない。

**是正案 (Viewport 単位のスクロールによる仮想ページング):**
印刷エンジンがない egui 環境で「自動ページ分割に委ねる」という設計意図（手動で切れ目を調整しない）を叶えるため、**「スライドページ＝画面の Viewport Height」** と定義する。
左右のキーボード操作（ページング）発生時に、正確に `1 Viewport Height` 単位でスクロールさせることで、連続した Markdown を「画面単位のページ」として振る舞わせる。これにより、diagram 等のレイアウト手動調整を回避しつつ、前後のページ移動を実現する。
