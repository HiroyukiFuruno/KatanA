...
- `crates/katana-linter/src/rules/domains/assets/svg.rs`
  - `assets/icons` 配下の SVG を white / none / currentColor 前提で検証している
...
### 3. built-in icon pack の asset 階層は `assets/icons/<pack-dir>/...` に固定する

icon pack ごとの管理境界を分かりやすくするため、built-in pack は `assets/icons/<pack-dir>/...` 配下に集約する。既存 icon は `katana-icon` pack だが、directory 名は短く `assets/icons/katana/...` とし、他 pack も同様に `assets/icons/material-symbols/...` のように並べる。

- 採用理由:
  - pack ごとの差分確認がしやすい
  - license inventory、coverage table、override 管理の参照先が明確になる
  - current `assets/icons` を親にしたまま移行でき、loader / linter の修正範囲を限定できる
- 代替案:
  - `assets/icon-packs/<pack-id>/...` に分離する: 既存 asset tooling との距離が増えるため不採用
  - `assets/icons/katana-icon/...` のように pack id と directory 名を完全一致させる: user-facing pack 名と filesystem path を不要に結びつけるため不採用

### 4. initial curated pack は `katana-icon` + external 5 種類に固定する
...
### 5. pack completeness は `Icon` enum 全件解決を基準にする
...
### 6. render policy は pack metadata で `TintedMonochrome` と `NativeColor` を切り替える
...
### 7. settings から pack selection を行い、即時反映する
...
### 8. license / provenance / override rationale は pack ごとに repository に残す
...
1. existing `assets/icons` を `assets/icons/katana/...` 配下へ再編し、`katana-icon` pack の asset root とする
