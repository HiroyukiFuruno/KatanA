...
- [ ] 3.6 【再指摘 p1】改善されていない、文字部分をクリックできない、ホバーしてもアクションが発生しない（ディレクトリ and ファイル名）
- [ ] 3.7 【再指摘 p1】ユーザー操作で横スクロールする場合も発生する必要はない、左右のボタン押下時のみにスクロール追従するように変更すること
- [ ] 3.8 【再指摘 p1】workspaceのfilterや目次表示などlightモードの全てのアイコンはbgをグレーにする指定が漏れている
- [ ] 3.9 【再指摘 p1】非同期かつ並列的なアプローチに本当になっているか？ユーザー想定（開く順にタブ増加）と異なり一斉に増えているように見えるため検証・改善
- [ ] 3.10 【保留】あと一つ思い出せない指摘あり（判明次第対応）
