...
- [ ] トグルで有効にした拡張子のファイルがワークスペースに表示されること
- [ ] 新規作成時にプルダウンから拡張子を選択できること
- [ ] `make check` が exit code 0 で通過
- [ ] Execute `/openspec-delivery` workflow (`.agents/workflows/openspec-delivery.md`) to run the comprehensive delivery routine (Self-review, Commit, PR Creation, and Merge).
...
- [ ] 4.1 `CustomTheme { name: String, colors: ThemeColors }` struct を定義
- [ ] 4.2 設定スキーマに `custom_themes: Vec<CustomTheme>`（最大10件）を追加
- [ ] 4.3 「名前をつけて保存」UIの実装（現在のカスタム色をテーマとして保存）
- [ ] 4.4 カスタムテーマプリセット一覧に保存済みテーマを表示する実装
- [ ] 4.5 カスタムテーマの削除UI（右クリックメニューまたは削除ボタン）
- [ ] 4.6 適用中のカスタムテーマが削除された場合のデフォルトへのフォールバック処理
