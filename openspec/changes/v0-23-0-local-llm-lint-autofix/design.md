...
ユーザー要望は「ユーザーが選べる local LLM」「軽くて容量が低いものが良い」であり、最初のスコープは lint 自動修正までである。したがって `v0.23.0` は bundled runtime ではなく local endpoint integration を前提とし、provider 選択と markdownlint autofix を同時に成立させる。
...
- remote commercial provider を `v0.23.0` の主対象にすること
...
local LLM の返す修正をそのまま silent apply すると Markdown を破壊する危険がある。したがって、`v0.23.0` の autofix は patch preview または修正候補確認の後で apply する。
...
`v0.19.0` で整備する official rule code、message、location、file path をそのまま prompt input / structured input に使い、追加 lookup を最小化する。
...
- batch autofix を `v0.23.0` で含めるか、single diagnostic fix を先行するか
