...
KatanA には locale JSON ベースの既存 i18n があり、静的 UI 文言はその仕組みで翻訳される。一方で、`v0.19.0` の markdownlint English description、`v0.23.0` と `v0.24.0` の local LLM result など、実行時に生成または外部由来で入る英語 text は locale JSON の外にある。

ユーザー要望は「local LLM がオンなら自動表示が望ましい」であり、ここで既存 i18n を置き換える必要はない。`v0.25.0` では、static UI は既存 i18n のまま維持し、dynamic / external English text にだけ translation overlay をかける。
