...
`v0.22.0` で local LLM provider と lint autofix の基盤が整えば、次の価値は文書作成そのものに local LLM を使えるようにすることにある。ユーザー要望では、新規ファイル生成、現在ファイルへの挿入、テンプレート生成を優先度なしで同時に扱いたい。
...
- `v0.22.0` の active local provider 設定と availability 判定を前提にする
