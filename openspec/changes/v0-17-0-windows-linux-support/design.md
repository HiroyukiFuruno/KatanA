...
- `v0.18.0` の正式サポート対象を macOS に加えて Windows x86_64 / Linux x86_64 へ広げる
...
### 1. `v0.18.0` の support matrix は `macOS + Windows x86_64 + Linux x86_64` に限定する

`Windows / Linux 対応` を曖昧にすると artifact 名、CI target、更新導線が確定しない。そこで `v0.18.0` は次の matrix に固定する。
...
`DiagramColorPreset` には Windows / Linux 向け candidate があるが、`OsFontScanner` は macOS 固定で、`font_loader` は emoji candidates を未使用のままにしている。`v0.18.0` では次を満たす。
...
- 採用理由:
  - 起動品質と文字表示品質を両立できる
- 代替案:
  - macOS と同じ emoji pipeline を各 OS で新規実装する: `v0.18.0` では重すぎるため不採用

### 7. update は `asset selection` と `install mode` を分離する

現在の update flow は「release tag を見る」「macOS zip を取る」「.app を swap する」が密結合している。`v0.18.0` ではこれを 2 段に分ける。
...
`v0.18.0` では installer の整備よりも、まず配布可能な artifact を揃えることを優先する。release artifact は次に固定する。
...
### 10. `正式サポート` の完了条件を CI / artifact / smoke verification の 3 点で固定する

`Windows / Linux 対応` は「コンパイルが通る」だけでは曖昧で、実装者ごとに完了条件が割れる。そこで `v0.18.0` での正式サポートは次をすべて満たす状態と定義する。

- CI 上で target OS の build または smoke verification が通る
- GitHub Releases に target OS 向け artifact が載る
- workspace を開く、Markdown を編集する、preview を表示する最低限の smoke verification 観点が文書化される
...
- **[Risk] updater を Windows / Linux で manual download にすると機能差が残る**  
  -> Mitigation: `v0.18.0` では broken auto-install を避けることを優先し、native installer は後続 change に分離する
...
3. update dialog と release asset naming を platform-aware に切り替える
4. release workflow と Makefile に Windows / Linux artifact を追加する
5. README / development guide / badge / support matrix を更新する
