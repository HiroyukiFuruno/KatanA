...
この検査がKatanA本体に閉じたままでは、KMMやUI widgetのような新規repositoryで同じ品質基準を維持できない。
...
- KMM、kdp、kle、kcf、kuwの着手条件へ組み込める状態にする。

## Non-Goals

- KMM文書モデルをAST lint側へ持たせること。
...
`katana-ast-lint` はP0とする。P1 `katana-markdown-model`、P2 `katana-ui-widget`、P3 その他より先に分離計画を固定する。
...
- 先にKMM実装へ入ると、KMM側に一時的な独自lintが増え、後から統一しにくくなる。
