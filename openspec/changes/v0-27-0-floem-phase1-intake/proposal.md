...
egui の IME 破損・カラー絵文字欠如・immediate mode CPU コストは、ユーザーが**入力時に最初に触れる痛み**である。Floem 移行は最優先の **入力サーフェス（editor + chat input）** から段階的に進める。本 change（Phase 1）では editor と chat 入力を Floem impl crate に差し替える。preview と chrome は後続 Phase（v0.28.0 / v0.29.0）で扱う。

検証範囲を絞るため、Floem 移行を 3 phase に分割する:

- **v0.27.0（本 change） = Phase 1: editor + chat input** ← 入力 IME / 絵文字を最優先で解決
- v0.28.0 = Phase 2: preview（vello retained 描画）
- v0.29.0 = Phase 3: chrome / eframe 完全除去
...
- preview は本 change では egui のままにする（v0.28.0 で扱う）
- chrome / toolbar / sidebar / split pane は本 change では egui / eframe のままにする（v0.29.0 で扱う）
