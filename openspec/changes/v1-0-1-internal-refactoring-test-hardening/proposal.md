...
- i18n formatter 抽象化: locale JSON による静的翻訳と、`katana-chat-ui` 経由の動的翻訳 overlay を統一する formatter 抽象を `katana-platform` に定義する（archived v0-30-0-advanced-i18n-runtime のスコープを吸収）。
...
- `i18n-formatter-abstraction`: 静的 locale JSON と動的翻訳 overlay を統一する formatter 抽象を `katana-platform` に定義する。
...
- `Justfile` / `just/*` / `scripts/runner/*` の test gate
