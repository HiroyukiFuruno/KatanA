...
`canvas-forge-intake` で図形描画と export の責務を kcf へ寄せた後、本設計では入力面の痛みを先に解くため、エディタ機能を `katana-language-editor` として独立した外部リポジトリに切り出す。
...
KatanA shell
  ...
  -> katana-canvas-forge       (version なしで取り込み済み)
