...
v0.27.0（Phase 1: editor + chat input）と v0.28.0（Phase 2: preview）で入力サーフェスと描画層を Floem に切り替えた後、本 change（Phase 3）は KatanA chrome（toolbar / sidebar / split pane / tab bar / window loop）を eframe / egui から Floem に完全移行し、`egui` / `eframe` / `egui_extras` / `egui_*` 系依存をゼロにする。

## DoR（Definition of Ready）

- v0.27.0（Phase 1）が完了している
- v0.28.0（Phase 2）が完了している
