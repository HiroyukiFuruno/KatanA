...
システムは、アプリケーションの再起動を要求せずに、runtime のテーマ変更をダイアグラムプレビュー描画へ反映しなければならない（SHALL）。kcf backed renderer を利用する Mermaid / Draw.io でも、KatanA の current theme が `RenderInput` 経由で kcf の実描画へ伝播しなければならない（MUST）。
...
#### Scenario: kcf の内部既定テーマが active theme を上書きしない

- **WHEN** KatanA が light theme の `RenderInput` を kcf へ渡す
- **THEN** kcf の内部 `DARK_MODE` や `DiagramColorPreset::current()` の状態は、KatanA が渡した light theme を上書きしない
- **THEN** KatanA は kcf 内部グローバル状態を同期するための隠れた呼び出しに依存しない
