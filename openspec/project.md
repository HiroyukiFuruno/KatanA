...
v0.26.0  katana-canvas-forge v0.1.0 intake（Mermaid + Draw.io + export 一括）

──── Floem 移行（egui 制約解消）────
v0.27.0  Floem Phase 1 intake = editor + chat input（IME / カラー絵文字解消・最優先）
v0.28.0  Floem Phase 2 intake = preview（vello retained 描画）
v0.29.0  Floem Phase 3 intake = chrome（toolbar / sidebar / split pane / window loop）+ egui / eframe ゼロ

──── post-Floem ────
v0.30.0  preview-driven local editing（review-first 編集体験）
v1.0.0   desktop viewer polish（公式リリース）
v1.0.1   internal refactoring（katana-ui 整理 + i18n formatter 抽象化）
```

各 phase は独立した release。検証範囲を狭めるため editor / preview / chrome を別 release として段階的に切り替える。
