...
version なし  katana-canvas-forge v0.1.0 intake（Mermaid + Draw.io + export 一括・master refactoring）

──── Floem 移行（egui 制約解消）────
v0.26.0  Floem Phase 1 intake = editor + chat input（IME / カラー絵文字解消・最優先）
v0.27.0  Floem Phase 2 intake = preview（vello retained 描画）
