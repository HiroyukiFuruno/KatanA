...
KatanA（シェル）
  ...
  └─ katana-canvas-forge (kcf) 外部描画バックエンド（Mermaid / Draw.io / PlantUML / math 等。egui 非依存）
```

### 依存方向の原則

- KatanA → 外部ライブラリ（一方向のみ）
- 外部ライブラリ同士は原則無依存。KDVはKCFの外部描画結果を受け取ってもよいが、同期制御は持たない。
...
version なし  katana-document-viewer rename / KME DTO viewer + export ownership 整理
