...
9. Task ごとの通常 PR push では、push 前に重い `just check` / `just check-light` を二重実行せず、通常の `git push` で `pre-push` hook を通す。`git push --no-verify` は禁止。hook や実行環境に問題がある場合は原因を直し、通常の `git push` で通す。
