...
1. If the current branch matches `feature/vX.Y.Z-task<N>` pattern → base is `release/vX.Y.Z`.
2. If the current branch matches `feature/vX.Y.Z-task<N>-<suffix>` pattern → base is still `release/vX.Y.Z`.
3. If the current branch matches `<feature>-task<N>` pattern → base is `<feature>` (the Base Feature Branch per `/openspec-branching`).
4. If the current branch matches `<feature>-task<N>-<suffix>` pattern → base is still `<feature>`.
5. If the current branch matches `<feature>-feedback`, `<feature>-fix`, or `<feature>-fixes` (e.g., `v0-7-0-feature-feedback-fixes`) pattern → base is STILL `<feature>` (the integration branch).
6. If the current branch IS the integration branch `<feature>` itself (e.g., `v0-7-0-feature` or `release/vX.Y.Z`) → base is `master`.
7. If the caller explicitly specifies a base branch, use that.
8. **If none of the above apply, ask the user.** Do not guess.
