...
- **(⚠️ 必須) PR作成前の機械的チェック**: PRを作成する前に、必ず `lefthook run pre-pr`（または `./scripts/release/check-pr-ready.sh`）を実行し、バージョン不整合やコミット漏れがないか確認してください。
- `create_pull_request` スキルを使用して対象のリリース用機能ブランチから `master` に向けたPRを作成します。
  - **推奨**: `gh release-pr --title "release: vX.Y.Z"` エイリアスを使用すると、上記チェックが自動で走り、不備がある場合は PR 作成が中断されます。
