...
3. **内部リンク（ローカルファイル・OpenSpec等）の全面禁止**:
    - CHANGELOGはWebサイト上に公開（レンダリング）されるため、リポジトリ内のローカルファイルやディレクトリ（例: `[OpenSpec - 〇〇](./openspec/...)`）への**相対パスや内部リンクを含めることは一切禁止**します。外部Webサイト上でリンク切れ（404 Not Found）を引き起こす致命的な原因となります。
    - **許可されるリンク**: URLが `http://` または `https://` から始まる完全な絶対パスであれば記載可能です。GitHubのPRリンク等の明示的なURLを含めたい場合は絶対パスを使用してください。
