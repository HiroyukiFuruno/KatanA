...
- **原則として手動でのバージョン変更が必要**: `Cargo.toml` と `crates/katana-ui/Info.plist` のバージョンを事前に手動で（または `bump-version.sh` で）更新し、GPG署名付きでプッシュしてください。CI上での自動更新は、署名漏れを防ぐためのバックアップ（`gh api` 経由）に限定されます。
...
`Cargo.toml` と `crates/katana-ui/Info.plist` の `version` を新しいリリースバージョン（例: `0.18.7`）に書き換えます。`make release` を使用すると全ての関連ファイルが一括更新されます。

```bash
# バージョン一括更新
make release VERSION=0.18.7

# 変更の確認
git diff

# コミット（署名必須）
git commit -S -m "release: v0.18.7"
git push origin release/v0.18.7
```
...
- ✅ Release Readiness（バージョン不整合チェック）
...
> 4. **Attestation**: 配布物に **GitHub Artifact Attestation** を付与し、出自を不変的に証明
> 5. **配布**: Homebrew Cask / Linuxbrew Formula / Winget レジストリへの自動公開
...
緊急事態や、何らかの理由で master ブランチから手動でリリースを再実行したい場合は、以下のコマンドを使用します：

```bash
# 基本的な手動トリガー
make release-trigger VERSION=0.18.7

# 既存のリリース・タグを強制上書き（再ビルド時）
make release-trigger VERSION=0.18.7 FORCE=1
```

> [!NOTE]
> 手動実行の場合、ワークフローの `preflight` ジョブが `Cargo.toml` のバージョン書き換えと `git push` を自動で行います。`FORCE=1` を指定すると、`gh release create --clobber` が適用され、既存のアセットを上書きして再公開します。

---

## 💡 トラブルシューティング

**前提条件**: `check-pr-ready.sh` により、不整合がある状態での PR マージは不可能です。ビルド前に不備が解消されていることが保証されます。
