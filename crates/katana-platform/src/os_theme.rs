...
/// Returns `Some(true)` if the OS is in dark mode, `Some(false)` for light mode,
...
fn detect_dark_mode_impl() -> Option<bool> {
    // Query NSApplication.effectiveAppearance via the Objective-C runtime.
...
fn detect_dark_mode_impl() -> Option<bool> {
    // Dark mode detection is not supported on non-macOS platforms.
    None
}
...
#[cfg(target_os = "macos")]
extern "C" {
    /// Returns the name of the NSApplication effective appearance as a C string.
    ///
    /// # Safety
...
mod tests {
    ...
    fn returns_some_bool_or_none_on_macos() {
        ...
        // On macOS: result is Some(true/false) when NSApp is available, or None when
        ...
        #[cfg(target_os = "macos")]
        // Either None (test env, no NSApp) or Some(bool) (running inside the app).
        let _ = result; // any value is acceptable in test environment
    }
}
