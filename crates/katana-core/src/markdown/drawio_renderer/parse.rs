...
use super::utils::{CANVAS_EDGE_MARGIN, CANVAS_MIN_HEIGHT, CANVAS_MIN_WIDTH, DrawioUtilsOps};

pub struct DrawioParseOps;
    ...
    pub fn estimate_canvas_size(cells: &[&Element]) -> (f64, f64) {
        ...
        for cell in cells {
            if let Some(geo) = cell.get_child("mxGeometry") {
                let x: f64 = DrawioUtilsOps::attr_f64(geo, "x");
                let y: f64 = DrawioUtilsOps::attr_f64(geo, "y");
                let w: f64 = DrawioUtilsOps::attr_f64(geo, "width");
                let h: f64 = DrawioUtilsOps::attr_f64(geo, "height");
