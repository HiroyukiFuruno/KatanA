use super::types::DrawioEdgeOps;
use xmltree::Element;

use crate::markdown::color_preset::DiagramColorPreset;

use super::types::{DrawioUtilsOps, Rect};
...
impl DrawioEdgeOps {
    ...
    pub fn render_edge(
        ...
        let Some((src, tgt)): Option<(Rect, Rect)> = Self::resolve_edge_rects(cell, geo_map) else {
            return;
        };
        let (scx, scy) = src.center();
        let (tcx, tcy) = tgt.center();
        let waypoints = Self::collect_waypoints(cell);

        let first_target = waypoints.first().copied().unwrap_or((tcx, tcy));
        let (x1, y1) = Self::border_point(&src, scx, scy, first_target.0, first_target.1);
        let last_source = waypoints.last().copied().unwrap_or((scx, scy));
        let (x2, y2) = Self::border_point(&tgt, tcx, tcy, last_source.0, last_source.1);

        let points_str = Self::build_polyline_points(x1, y1, &waypoints, x2, y2);
        shapes.push_str(&format!(
            "<polyline points=\"{points_str}\" fill=\"none\" stroke=\"{}\" stroke-width=\"1.5\" marker-end=\"url(#katana-arrow)\"/>",
            preset.arrow
        ));

        Self::append_edge_label(cell, shapes, x1, y1, x2, y2, preset);
    }
    ...
    fn collect_waypoints(cell: &Element) -> Vec<(f64, f64)> {
        ...
        for child in &geo.children {
            ...
            for pt_node in &el.children {
                ...
                let x = DrawioUtilsOps::attr_f64(pt, "x");
                let y = DrawioUtilsOps::attr_f64(pt, "y");
                points.push((x, y));
            }
        }
        points
