...
fn different_strings_return_different_hashes() {
    assert_ne!(
        ShellLogicOps::hash_str("hello"),
        ShellLogicOps::hash_str("world")
    );
    assert_ne!(ShellLogicOps::hash_str("a"), ShellLogicOps::hash_str("b"));
}

#[test]
fn non_ascii_string_hash_is_consistent() {
    let h1 = ShellLogicOps::hash_str("hello world");
    let h2 = ShellLogicOps::hash_str("hello world");
    assert_eq!(h1, h2);
    assert_ne!(h1, ShellLogicOps::hash_str("goodbye"));
}

#[test]
fn identical_inputs_always_return_same_hash_value() {
    let input = "test-consistency-check";
    let results: Vec<u64> = (0..100).map(|_| ShellLogicOps::hash_str(input)).collect();
...
fn returns_relative_path_if_root_is_provided() {
    ...
    assert_eq!(
        ShellLogicOps::relative_full_path(path, Some(root)),
        "specs/auth/spec.md"
    );
}
...
fn returns_full_path_if_no_root_provided() {
    ...
    assert_eq!(
        ShellLogicOps::relative_full_path(path, None),
        "/home/user/doc.md"
    );
}
...
fn returns_empty_string_if_path_and_root_are_identical() {
    ...
    let root = Path::new("/workspace");
    assert_eq!(ShellLogicOps::relative_full_path(path, Some(root)), "");
}
...
fn test_format_tree_tooltip_metadata_unavailable() {
    ...
    let tooltip = ShellLogicOps::format_tree_tooltip("metadata_unavailable.md", path);
