...
fn font_size_change_is_reflected_in_settings() {
    /* WHY: Verify that updating the font size in the app configuration immediately impacts the effective (clamped) font size. */
...
fn font_size_out_of_range_is_clamped_in_app() {
    /* WHY: Safety check: Verify that extreme font size values (e.g., 99.0 or 1.0) are correctly clamped to the application-defined safe range (8.0 to 32.0). */
    let state = AppState::new(
        katana_core::ai::AiProviderRegistry::new(),
        katana_core::plugin::PluginRegistry::new(),
        katana_platform::SettingsService::default(),
