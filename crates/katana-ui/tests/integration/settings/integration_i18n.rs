...
fn wait_for_workspace_load(harness: &mut Harness<'static, KatanaApp>) {
    /* WHY: Utility to block the test until the async workspace loading process completes. */
...
fn setup_harness_with_json_repo(settings_path: &std::path::Path) -> Harness<'static, KatanaApp> {
    /* WHY: Factory to create a test harness with a persistent JSON-backed settings repository. */
