...
fn fixture_ja_s5_5_short_long_short_columns_wrap_properly() {
    ...
    let text_width = bounds.x1 - bounds.x0;
    // With the proportional width allocation, the long column should take up the lion's share
    // of the 600px width, while the short columns shrink.
    assert!(
        (350.0..=590.0).contains(&text_width),
        "The mixed column table should give the long column more width. Width was {:.1}px",
        text_width
    );
    ...
    assert!(
        text_height > 20.0,
        "The text should still wrap, but height was {:.1}px",
        text_height
    );
}
...
";
    ...
    let mut harness = Harness::builder()
        ...
        .build_ui(move |ui| {
            ...
            p.show_content(ui, None, None, None, None);
        });
    for _ in 0..3 {
        harness.step();
    }
...
fn red_text_click_toggles_task_state() {
    ...
    let mut harness = Harness::builder()
        ...
        .build_ui(move |ui| {
            ...
            let (_req, actions) = test_pane.show_content(ui, None, None, None, None);
            if !actions.is_empty() {
...
fn test_multiple_siblings() {
    ...
    let mut harness = Harness::builder()
        ...
        .build_ui(move |ui| {
            ...
            let mut test_pane = PreviewPane::default();
            test_pane.sections = sections.clone();
            test_pane.show_content(ui, None, None, None, None);
        });
    harness.step();
}
