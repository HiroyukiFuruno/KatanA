...
impl BehaviorTabOps {
    pub(crate) fn render_behavior_tab(
        ...
        ui.add_space(SUBSECTION_SPACING);
        performance::BehaviorPerformanceOps::render(ui, state);
        ui.add_space(SUBSECTION_SPACING);
        ...
        ui.indent("slideshow_settings_body", |ui| {
        ...
        ui.label(egui::RichText::new(&behavior_msgs.startup_behavior_section_title).strong());
        ui.add_space(SETTINGS_TOGGLE_SPACING);

        ui.indent("startup_behavior_settings_body", |ui| {
        ...
        ui.indent("cache_settings_body", |ui| {
            clear_cache = ui.button(&behavior_msgs.clear_http_cache).clicked();
        });
        if clear_cache {
