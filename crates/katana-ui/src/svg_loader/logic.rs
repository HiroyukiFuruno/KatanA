...
impl KatanaSvgLoader {
    ...
    pub fn install(ctx: &Context) {
        ...
        ctx.include_bytes(
            "bytes://icon/copy.svg",
            include_bytes!(
                "../../../../vendor/egui_commonmark_upstream/egui_commonmark_backend/src/copy.svg"
            ),
        );
        ctx.include_bytes(
            "bytes://icon/success.svg",
            include_bytes!(
                "../../../../vendor/egui_commonmark_upstream/egui_commonmark_backend/src/check.svg"
            ),
        );
...
mod tests {
    ...
    fn is_supported_accepts_svg_with_query_string() {
        assert!(KatanaSvgLoader::is_supported(
    ...
    fn preprocess_svg_bytes_prefers_apple_color_emoji_for_badges() {
        ...
        let processed = KatanaSvgLoader::preprocess_svg_bytes(svg.as_bytes()).expect("svg text");
    ...
    fn rasterize_svg_bytes_renders_sponsor_badge_logo_and_emoji() {
        ...
        let processed =
            KatanaSvgLoader::preprocess_svg_bytes(svg.as_bytes()).expect("processed badge svg");
        let mut options = resvg::usvg::Options::default();
        options.fontdb_mut().load_system_fonts();
        let image = KatanaSvgLoader::rasterize_svg_bytes_with_size(
            processed.as_bytes(),
    ...
    fn test_svg_rasterize_fallback_size() {
        ...
        let result = super::KatanaSvgLoader::rasterize_svg_bytes_with_size(
            svg_data.as_bytes(),
