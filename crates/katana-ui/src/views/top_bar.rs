...
use crate::shell_ui::{
    invisible_label, open_folder_dialog, relative_full_path, LIGHT_MODE_ICON_BG,
    STATUS_BAR_ICON_SPACING, STATUS_SUCCESS_GREEN,
};
use eframe::egui;

pub(crate) struct MenuBar<'a> {
...
impl<'a> MenuBar<'a> {
    ...
    pub fn show(self, ctx: &egui::Context) -> egui::InnerResponse<()> {
        egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
            egui::MenuBar::new().ui(ui, |ui| {
                ui.menu_button(crate::i18n::get().menu.file.clone(), |ui| {
                    FileMenu::new(self.state, self.action).show(ui);
                });
                ui.menu_button(crate::i18n::get().menu.settings.clone(), |ui| {
                    SettingsMenu::new(self.state, self.action).show(ui);
                });
                ui.menu_button(crate::i18n::get().menu.help.clone(), |ui| {
                    HelpMenu::new(self.action).show(ui);
                });
                HeaderRight::new(self.state).show(ui);
            });
        })
    }
}

pub(crate) struct HelpMenu<'a> {
    pub action: &'a mut AppAction,
}

impl<'a> HelpMenu<'a> {
    pub fn new(action: &'a mut AppAction) -> Self {
        Self { action }
    }

    pub fn show(self, ui: &mut egui::Ui) {
        if ui.button(crate::i18n::get().menu.about.clone()).clicked() {
            *self.action = AppAction::ToggleAbout;
            ui.close();
        }
        if ui
            ...
            .clicked()
        {
            *self.action = AppAction::ShowReleaseNotes;
            ui.close();
        }
    }
}

pub(crate) struct FileMenu<'a> {
...
impl<'a> FileMenu<'a> {
    ...
    pub fn show(self, ui: &mut egui::Ui) {
        ...
        if ui
            ...
            .clicked()
        {
            *self.action = AppAction::SaveDocument;
            ui.close();
        }

        let has_doc = self.state.active_document().is_some();
        ui.add_enabled_ui(has_doc, |ui| {
            ui.menu_button(crate::i18n::get().menu.export.clone(), |ui| {
                if ui
                    ...
                    .clicked()
                {
                    *self.action = AppAction::ExportDocument(crate::app_state::ExportFormat::Html);
                    ui.close();
                }
                if ui
                    ...
                    .clicked()
                {
                    *self.action = AppAction::ExportDocument(crate::app_state::ExportFormat::Pdf);
                    ui.close();
                }
                if ui
                    ...
                    .clicked()
                {
                    *self.action = AppAction::ExportDocument(crate::app_state::ExportFormat::Png);
                    ui.close();
                }
                if ui
                    ...
                    .clicked()
                {
                    *self.action = AppAction::ExportDocument(crate::app_state::ExportFormat::Jpg);
                    ui.close();
                }
            });
        });
...
pub(crate) struct SettingsMenu<'a> {
    pub _state: &'a AppState,
    pub action: &'a mut AppAction,
}

impl<'a> SettingsMenu<'a> {
    pub fn new(_state: &'a AppState, action: &'a mut AppAction) -> Self {
        Self { _state, action }
    }
        ...
        ui.menu_button(crate::i18n::get().menu.language.clone(), |ui| {
            ...
            for (code, display_name) in crate::i18n::supported_languages() {
                if ui.button(display_name.as_str()).clicked() {
                    *self.action = AppAction::ChangeLanguage(code.to_string());
                    reset_layout = true;
                }
            }
            if reset_layout {
...
pub(crate) struct HeaderRight<'a> {
    pub state: &'a AppState,
}

impl<'a> HeaderRight<'a> {
