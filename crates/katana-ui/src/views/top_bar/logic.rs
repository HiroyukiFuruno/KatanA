...
impl TopBarOps {
    ...
    pub fn find_best_drop_index(
        drop_points: &[(usize, f32)],
        ghost_center_x: f32,
    ) -> Option<usize> {
    ...
    pub fn resolve_drag_drop(
        ...
        let drop_points = Self::compute_drop_points(tab_rects);
        if let Some(to) = Self::find_best_drop_index(&drop_points, ghost_center_x)
            && src_idx != to
            && src_idx + 1 != to
        {
...
mod tests {
    ...
    fn compute_drop_points_empty() {
        assert!(TopBarOps::compute_drop_points(&[]).is_empty());
    }
    ...
    fn find_best_drop_index_nearest() {
        ...
        assert_eq!(TopBarOps::find_best_drop_index(&points, 110.0), Some(1));
        assert_eq!(TopBarOps::find_best_drop_index(&points, 190.0), Some(2));
    }

    #[test]
    fn find_best_drop_index_empty() {
        assert_eq!(TopBarOps::find_best_drop_index(&[], 50.0), None);
    }

    #[test]
    fn resolve_drag_drop_same_position_no_action() {
        let rects = make_rects(&[(10.0, 100.0), (110.0, 200.0)]);
        assert!(TopBarOps::resolve_drag_drop(0, 30.0, &rects).is_none());
    }

    #[test]
    fn resolve_drag_drop_adjacent_no_action() {
        let rects = make_rects(&[(10.0, 100.0), (110.0, 200.0)]);
        assert!(TopBarOps::resolve_drag_drop(0, 105.0, &rects).is_none());
    }
    ...
    fn resolve_drag_drop_reorder_action() {
        ...
        assert_eq!(
            TopBarOps::resolve_drag_drop(0, 260.0, &rects),
