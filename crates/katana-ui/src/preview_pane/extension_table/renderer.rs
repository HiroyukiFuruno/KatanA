...
/* WHY: Keep heading->table margin stable when decoration layer lifts table border
 * by a small top bleed. */
const TABLE_TOP_SPACING: f32 = 14.0;
...
impl KatanaTableRenderer {
    pub fn render<'e>(
        ...
        ui.add_space(TABLE_TOP_SPACING);
                            ...
                            .show(inner_ui, |grid_ui| {
                                ...
                                for row_idx in 0..table_data.rows.len() {
                                    if row_idx % 2 == 1 {
                                        row_bg_indices.push((
                                            row_idx,
                                            grid_ui.painter().add(egui::Shape::Noop),
                                        ));
                                    }
                                }
