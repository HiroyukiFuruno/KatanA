...
KatanA は Markdown のフェンスコードブロック内の Mermaid、PlantUML、Draw.io ダイアグラムをサポートしています。Mermaid と Draw.io の描画用ファイルは KatanA が自動で準備します。PlantUML のレンダリングには Java ランタイムが必要です。
...
PlantUML のレンダリングに必要な Java ランタイムを Windows にインストールする手順です。
