...
- **ネイティブ文書表示** — HTML、PDF、DOCX、XLSX、PPTXのローカルファイルと直接文書URLをワークスペース内で表示
...
KatanA は Markdown のフェンスコードブロック内の Mermaid、PlantUML、Draw.io ダイアグラムをサポートしています。ダイアグラム描画は同梱の KDV / KRR 連携で処理されるため、Java ランタイムや描画用ファイルのダウンロード手順は不要です。
